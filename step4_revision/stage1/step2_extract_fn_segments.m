...
fn = readtable('./data/segments_to_review/fn_segments.csv', 'HeaderLines',1, 'ReadRowNames', 0);

recs = convertCharsToStrings(fn.Var4);
recordings = unique(recs);
epochs = [fn.Var1, fn.Var2, fn.Var3];
true_classes = fn.Var7;
fn_model_predictions = fn.Var8;

df = table;
for r=1:size(recordings)
    inds = find(recs==recordings(r));
    tmp = table(fn.Var2(inds), fn.Var3(inds), convertCharsToStrings(fn.Var4(inds)), ...
        fn.Var5(inds), convertCharsToStrings(fn.Var6(inds)), fn.Var7(inds), fn.Var8(inds));
    df = [df;tmp];
end
df.Properties.VariableNames = {'start_time', 'end_time', 'filename', 'probability', 'true_label', 'true_class', 'predicted_class'};
writetable(df, './data/df_fn_segments.txt');
...
for rIdx = 1 : length(recordings)
    ...
    selected_predictions = fn_model_predictions(inds_epochs, :);
...
save(fullfile(folder_output, 'fn_segments.mat'), 'recordings', 'recording', ...
